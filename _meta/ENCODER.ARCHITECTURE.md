...
- **Gensing-format job specs (Markdown body, `.spec` extension):** `.3ox/(3)Rules/exc/tpw/*.spec` — line-1 **`::0x000::` … `::0x003::`**, **`⫸`** context, **`▞▞ JOB`** per block; hex rows **`0x000`–`0x003`** in **`hex.index.json`**. Spin order in doc: **k00 Warden → k01 Tape → k02 Pulse** (matches **`core.axis`** map).
